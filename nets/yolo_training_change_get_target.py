# 这个py文件是在原先"yolo_training.py"的基础上面修改了self.get_target函数,修改的动机为每一个尺度（13*13， 26*26, 52*52）下，每一个真实框都要有一个先验框来负责（也就是
# 说在三种尺度下，要有三个先验框来负责预测）。
# 而实际上，人家yolov3的作者的想法为：所有的真实框在所有尺度（13*13， 26*26, 52*52）只要有一个先验框来负责预测即可。
